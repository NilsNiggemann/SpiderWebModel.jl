...
TilingMatrix(BasisStates,Tiles::AbstractMatrix,shape::Tuple{Int,Int}) = TilingMatrix(BasisStates,Tiles,size(BasisStates[begin]),shape)
...
function constructGSFromTiles_Threads(Tiles,Lx,Ly;numTries = 100000)
    ...
    Threads.@threads for i in 1:nThreads
        StatesCollection[i] = constructGSFromTiles(Tiles,Lx,Ly,numTries=numTries÷nThreads)
    end
