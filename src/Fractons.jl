...
function getPlotPoints(State)
    ...
    cols = [col(i,j) for i in axes(ChargeMat,1),j in axes(ChargeMat,2) if ChargeMat[i,j] ≠ 0]
    sizes = [size(i,j) for i in axes(ChargeMat,1),j in axes(ChargeMat,2) if ChargeMat[i,j] ≠ 0]

    points = [Point2(i,j) for i in axes(ChargeMat,1),j in axes(ChargeMat,2) if ChargeMat[i,j] ≠ 0]
