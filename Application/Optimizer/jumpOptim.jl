...
##
for i in 1:Lx  # For each row
    for j in 1:Ly  # and each column
        ...
        @constraint(spiderweb, x[i, j + 1]+x[i - 1, j + 1]-x[i - 1, j]-x[i - 1, j - 1]+x[i, j - 1]+x[i + 1, j - 1]-x[i + 1, j]-x[i + 1, j + 1] == 0)
    end
end
...
function getSols(N,spiderweb,x,Lx,Ly)
    success = falses(N)
    sols = [BitMatrix(zeros(Bool,Lx,Ly)) for _ in 1:N]
    ...
    for iter in 1:N
        ...
        println("step ",iter , " out of ", N ," progress: ",iter/N*100, "%", " success: ", step_successful)
