...
function setUpSpiderWeb(L;OutputFlag=false,TimeLimit=3*60,LogFile="ConfigSearch_$L.log")
    ...
    for i in 1:Lx
        for j in 1:Ly
            ...
            @constraint(model, y[i, j + 1]+y[i - 1, j + 1]-y[i - 1, j]-y[i - 1, j - 1]+y[i, j - 1]+y[i + 1, j - 1]-y[i + 1, j]-y[i + 1, j + 1] == 0)
        end
    end
...
function multipleRuns(model,L,NTot,NIndiv,initializeDenominator=12)
    ...
    @showprogress dt=0.1 desc="obtain configs..." for iter in 1:NTot
...
##
L = 40
model = setUpSpiderWeb(L,OutputFlag=true,TimeLimit=15,LogFile="ConfigSearch_$L.log")
