...
#SBATCH --output=/scratch/hpc-prf-pm2frg/niggeni/JobsOutput/Spiderweb/SR/stair_%a.out
...
julia -O3 -t $SLURM_CPUS_PER_TASK --heap-size-hint=210G /pc2/groups/hpc-prf-pm2frg/niggeni/Jobs/SpiderWebModel.jl/Application/GFMC/StairCase_Sweep.jl/StochRec_stairFine.jl $SLURM_ARRAY_TASK_ID
exit
=#
...
NStepsEnd = 30
NBins = 3000
stoch_rec_learning_rate = 3e-8

NWalkers_stochRec = Threads.nthreads() * 3
equilibration_steps_stochRec = 1000
report_steps_SR = 5
##
# -- debug params --
if isinteractive()
...
if !isfile(outfileSR)
    @info "starting run" L τ=CT_stochRec.τ NStepsEnd NWalkers_stochRec outfileSR
    stochReconfRes = SW.stochastic_reconfiguration(parentState,CT_stochRec,NStepsEnd,ψGSymm,NBins,stoch_rec_learning_rate,SW.IterativeSRSolver();Nwalkers = NWalkers_stochRec,rel_tolerance=0.,equilibration_steps=equilibration_steps_stochRec,pre_equilibration_steps=1_000_000,scatter_fraction,outfile=outfileSR,reset = false,report_steps = report_steps_SR)
