...
function makeRun(S,μ,folder;Nwalkers = 30,NSteps = 8000,NwalkersOpt = 1,NStepsOpt = NSteps,OptIndep = Threads.nthreads(),kwargs...)
    ...
    if !isdir(files_folder)
        ...
        CT2 = SW.ContinuousTimeMethod(0.08,1,(1-μ)* 0.266*length(S),SW.Hxx_RK(μ))
        
        @time results = fetch.([Threads.@spawn SW.startManyWalkerGFMC(S,CT2,Nwalkers,NSteps,ψG;equilibration_steps=0,outfile = joinpath(files_folder,"$i.h5"),kwargs...) for i in 1:6])
        try
...
##
folder = "/p/scratch/pmfrg/niggemann1/LoopSector_noOpt/L=$(size(S,1))/noString"
makeRun(S,μ,folder;Nwalkers = 48*50,NSteps = 5000,NwalkersOpt = 48*80,NStepsOpt = 400,OptIndep = 20,equilibration_steps=800)
