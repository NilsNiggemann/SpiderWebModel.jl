...
function plotChargeFFT(ChargeFFT;kwargs...)
    with_theme(theme_PiTicks()) do
        ...
        hm = heatmap!(ax,k,k,ChargeFFT,colormap = :balance,colorrange = ±(ChargeFFT);kwargs...)
...
StaggeredMag(Conf) = sum((-1)^(x+y)*Conf[x,y] for x in axes(Conf,1),y in axes(Conf,2))
...
function deconvolve(Charge, kernelFFT,S_tot = 0,SStag =0;normalize=true)
    ...
    for i in 1:size(SpinFFT,1), j in 1:size(SpinFFT,2)
        isnan(SpinFFT[i,j]) && (SpinFFT[i,j] = 0)
    end
    @info "" 2*sum(real, SpinFFT) sum(imag,SpinFFT) size(SpinFFT) 2Mag(SpinFFT) StaggeredMag(SpinFFT) S_tot SStag
...
with_theme(theme_PiTicks()) do 
    ...
    hm = heatmap!(axabs,k,k,abs.(kernelFFT_large).+1e-20,colormap = :viridis,colorscale = log10)
...
kernelRotated(kx,ky) = kernelAnalytical(
...
with_theme(theme_PiTicks()) do
    ...
    ax2 = Axis(fig[1, 2],title = L"$|c(\mathbf{k})|$",aspect = 1,yticksvisible = false,yticklabelsvisible = false)
...
function sampleCharges(N)
    getCharge() = conv(SW.constructConfigPath(20,20,SW.ALLGS_S12,SW.spiralPath,maxiter = 100000, verbose = false),iceRule)
    charges= fetch.([Threads.   @spawn getCharge() for i in 1:N])
...
function getStructureFac(ChargeFFT)
    ...
    return [mean(C[i,j]^2. for C in ChargeFFT)/c(kx,ky)^2 for (i,kx) in enumerate(k), (j,ky) in enumerate(k)]
