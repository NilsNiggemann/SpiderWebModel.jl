...
Proj(L) = sum((Proj(n, m, L) for n = 1:2 for m = 1:2))
...
function SpiderWebBasis()
    ...
    a2 = SA[-1, 1]
    b = [SA[0, 0], SA[1, 0]]
    return Basis_Struct_2D(; a1, a2, b, NNdist = 1.0, NUnique = 2)
end
...
##
function printCouplings(p0)
    AllTerms = inBoxCouplings(p0)
    println("             Couplings for ", p0, "            ")
    for term in AllTerms
        site = (term.n1, term.n2)
        println(term.fac, " × ", "S$site")
    end
