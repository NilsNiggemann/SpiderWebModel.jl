...
nThermal = 300
##
@time res = SW.startManyWalkerGFMC(S,48,1000,12,ψG,1;equilibration_steps=nThermal)
##
a = SW.reconf_obs(S,eachslice(res.SaveConfigs,dims=(3,4)),ψG,1)
##
SW.stochastic_reconfiguration(S,100,ψG,5,3e-2;Nwalkers = 10,nbra = 2,rel_tolerance=1e-1,equilibration_steps=nThermal)
##
SW.Random.seed!(1234)
stochReconfRes = SW.stochastic_reconfiguration(S,200,ψG,20,3e-2;Nwalkers = 20,nbra = 10,rel_tolerance=1e-3,equilibration_steps=nThermal)
##
...
##
stochReconfRes_2 = SW.stochastic_reconfiguration(S,1000,ψGnew,10,1e-3;Nwalkers = 60,nbra = 40,rel_tolerance=1e-1,equilibration_steps=nThermal)
...
@time results = fetch.([Threads.@spawn SW.startManyWalkerGFMC(S,40,30_000÷nBra,nBra,ψGnew,1;equilibration_steps=nThermal,pre_equilibration_steps=nBra*nThermal,w_avg_estimate = 8.) for _ in 1:12])
##
nBraOld = 10
SW.Random.seed!(1232)
@time resultsOld = fetch.([Threads.@spawn SW.startManyWalkerGFMC(S,40,30_000÷nBraOld,nBraOld,ψGold,1;equilibration_steps=nThermal,pre_equilibration_steps=nBra*nThermal,w_avg_estimate = 8.) for _ in 1:12])

##
# plotEnergies(results,nBra,-20.35;Emin=-20.5,Emax=-19.8) # L=10
plotEnergies(resultsOld,nBraOld,p=400)
plotEnergies!(results,nBra;p=400,color=:red) # L=15
current_figure()
