...
function getEnergy(S,mu,Symmetry;Nwalkers = 20 * 3,NSteps = 2000,nThermal = 1000,NRuns=20,dt = 3e-3,NConfs = 20,nopt=1000,pre_init_steps=800)
    ...
    initializer = SW.VariableTimePropagation(LinRange(5,tau,pre_init_steps),CT)
...
L = 18
Symm = SW.TranslationalSymmetry([6,0],[0,6]) #10

output_dir = "../../Data/EnergyScaling_S1/6x6_L$L"
...
S = SW.stencilConfig(1. *SW.periodicState6x6Condensate(L),1)
##
for mu in muRange
    mu_file = joinpath(output_dir, "mu_$(round(mu, digits=3)).h5")
    if isfile(mu_file)
        println("mu = $mu skipped")
        continue
    end
    try
        h5write(mu_file, "sector", "6x6")
