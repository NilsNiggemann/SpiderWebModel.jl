...
##
Confs = [SW.stencilConfig(float(S),1) for S in eachslice(h5read("../../Data/Spin1Confs/Confs_40_1.h5","Confs/18"),dims=3)]
Confs2 = [SW.stencilConfig(float(S),1) for S in eachslice(h5read("../../Data/Spin1Confs/Confs_40_2.h5","Confs/18"),dims=3)]
...
##
CT = SW.ContinuousTimeMethod(20,Hxx = SW.Hxx_RK(1),w_avg_estimate=0.)

ObsRuns = fetch.([Threads.@spawn SW.measure_Sq_GFMC(sConfs[1],CT,1,10000,2,ψG,pre_equilibration_steps=50_000,equilibration_steps=500,estimate_w_avg=false,scatter_fraction=0.95) for i in 1:1*10])
