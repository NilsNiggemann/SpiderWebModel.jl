...
if !isfile(SRoutfile)
    stochReconfRes = SW.stochastic_reconfiguration(S,CTSR,50,ψGSymm,1000,3e-3,SW.IterativeSRSolver();Nwalkers = 28*8,rel_tolerance=1e-8,equilibration_steps=1000,pre_equilibration_steps=1_000,report_steps=10,outfile = SRoutfile)
...
outdir = "../../Data/open_L$(L)/"
rm(outdir,recursive=true,force=true)
mkdir(outdir)
CT = SW.ContinuousTimeMethod(0.1,w_avg_estimate = w_avg_estimate,Hxx = CTSR.Hxx)
##
ObsRuns = fetch.([Threads.@spawn SW.measure_Sq_GFMC(S,CT,28*300,4000,150,ψG,estimate_w_avg=true,equilibration_steps=2500,pre_equilibration_steps=10_000,outfile = "$outdir/$i.h5") for i in 1:12])
