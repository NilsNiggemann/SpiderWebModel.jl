...
function getConfigs(L,numConfigs = 10;kwargs...)
    ...
    S = fetch.([Threads.@spawn SW.constructConfigPath(SW.DictAlgorithm(),L,L,SW.ALLGS_S12, setup;maxiter,deleteSteps = SW.getStepDeleter(L+2,1,15),verbose = false,kwargs...) for _ in 1:numConfigs])
...
with_theme(theme_PiTicks()) do 
    fig,ax,hm = heatmap(sum(edSol.Sq_ks[i] for i in eachindex(edSol.En));axis = (;aspect=1,title = L"μ = %$μ"),figure = (;size = 0.8 .*(400,300)))
