...
with_theme(theme_SimpleTicks()) do
    S = SW.SpinConfig(0.5 .*ones(7,5),1/2)
    S[5:2:end,1:2:3] .= -0.5
...
##
with_theme(theme_SimpleTicks()) do
    S = SW.SpinConfig(0.5 .*ones(7,5),1/2)    # S = SW.SpinConfig([iseven(i+j)-0.5 for i in 1:10,j in 1:10],1/2)
...
with_theme(theme_SimpleTicks()) do
    # global i +=1
    SW.Random.seed!(34)
    S = SW.constructConfigPath(5,5,SW.ALLGS_S12,)
    # S = SW.SpinConfig(S[1:10,1:10],1/2)
